...
function CitationTag({ c }: { c: Citation }) {
  return (
    <span
      className={cn(
        "inline-flex items-center font-mono text-[9px] tracking-wide px-1.5 py-0.5 rounded border shrink-0",
        c.label === "A"
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-3 pb-2 shrink-0">
        <p className="font-mono text-[9px] text-muted-foreground/50 tracking-widest uppercase">
          Memory preserved · Citations included
        </p>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-2 space-y-3 min-h-0">
        {messages.length === 0 ? (
          <div className="space-y-1.5">
            <p className="font-mono text-[9px] text-muted-foreground/40 tracking-widest uppercase mb-3">
              Suggested questions
            </p>
            {SUGGESTIONS.map((q) => (
              <button
                key={q}
                onClick={() => send(q)}
                className="w-full text-left text-xs text-muted-foreground hover:text-foreground bg-white/[0.02] hover:bg-white/[0.04] border border-border/30 hover:border-border/60 rounded-lg px-3 py-2 transition-all leading-relaxed"
              >
                {q}
              </button>
            ))}
